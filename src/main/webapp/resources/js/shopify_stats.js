(function () {
    ...
    function readStorage(permanent, cookieName) {
        ...
        if (storage in window) {
            return window[storage].getItem(cookieName);
        } else {
            return false;
        }
    }
    ...
    function setStorage(permanent, cookieName, cookieValue) {
        var storage = permanent ? 'localStorage' : 'sessionStorage';
        if (storage in window) {
            window[storage].setItem(cookieName, cookieValue);
        }
    }
    ...
    function fetch(permanent, cookieName, func) {
        ...
        if (!cookie || !permanent) {
            setCookie(permanent, cookieName, cookieValue);
        }
        if (!local) {
            setStorage(permanent, cookieName, cookieValue);
        }
    ...
    if (window.__st && !window.__st['offsite'] && window.location.host != 'checkout.shopify.com') {
        ...
        if (readCookie('_shopify_visit')) {
            visit = 1;
        }
        if (readCookie('_shopify_uniq')) {
            uniq = 1;
        }
        ...
        if (readCookie('_shopify_s')) {
            visit = 1;
        }
        if (readCookie('_shopify_y')) {
            uniq = 1;
        }

    }

    uniqToken = fetch(true, '_shopify_y', function () {
        return uniqueId();
    });
    visitToken = fetch(false, '_shopify_s', function () {
        return uniqueId();
    });
